...
import { countryFilter, dealershipClassFilters, STATE_PROPERTY } from "./filters";
import { SYSTEM_OWNERS } from "@/config/roster";
import type { OverviewResponse, FilterScope, PropertyFilter } from "./types";
...
// this responds in a couple seconds even cold. Shared between /api/overview
// and the daily history job (history.ts) so both use the exact same
// definition of the 5 headline KPI numbers.
export async function computeOverview(scope: FilterScope): Promise<OverviewResponse> {
